...
class Orthomcl():
	def __init__(self, input_fasta, index_mysql_tables, working_dir, BLAST_file, evalue_exponent, percent_match_cutoff, inflation_value):
		...
		if not BLAST_file:
			...
			script = generate_bsub_file.BSUB_script(command=ortho_blast_parser, mem_in_GB=2, name="parsing", log_file="out_BlastParser.txt", error_file="err_BlastParser.txt")
		...
		script = generate_bsub_file.BSUB_script(command=ortho_load_blast, mem_in_GB=2, name="load blast", log_file="out_LoadBlast.txt", error_file="err_LoadBlast.txt")
		generate_bsub_file.run_job_and_wait(script)

		print "orthomclPairs"
		ortho_pairs = "orthomclPairs orthomcl_config.txt orthomcl_pairs.log cleanup=yes"
		script = generate_bsub_file.BSUB_script(command=ortho_pairs, mem_in_GB=2, name="pairs", log_file="out_Pairs.txt", error_file="err_Pairs.txt")
		generate_bsub_file.run_job_and_wait(script)
		...
		script = generate_bsub_file.BSUB_script(command=ortho_dump, mem_in_GB=2, name="dump", log_file="out_DumpPairsFiles.txt", error_file="err_DumpPairsFiles.txt")
		generate_bsub_file.run_job_and_wait(script)

		print "mcl"
		ortho_mcl = "mcl mclInput --abc -I %s -o mclOutput" % inflation_value
		script = generate_bsub_file.BSUB_script(command=ortho_mcl, mem_in_GB=2, name="mcl", log_file="out_mcl.txt", error_file="err_mcl.txt")
		generate_bsub_file.run_job_and_wait(script)

		print "drop database"
		cmd = "drop.sh orthomcl orthomcl orthomcl"
		script = generate_bsub_file.BSUB_script(command=cmd, mem_in_GB=2, name="drop", log_file="out_drop.txt", error_file="err_drop.txt")
		generate_bsub_file.run_job_and_wait(script)
	...
	def run_blast(self, fasta_file, evalue_string):
		...
		generate_bsub_file.run_job_and_wait(script)
		job_ids = []
		for i in range(1, n_files+1): # old cutoff: 0.00001 | giant virus  0.01 | kleb 0.00001 | giant virus mai 15 : 0.1
			cmd = "blastp -db %s -query group_%s.fasta -out blast_result_%s.txt -outfmt 6 -num_threads 8 -evalue %s" % (fasta_file, i, i, evalue_string)
...
# original cutoffs: 50%, evolue -5 giant virus 50/-2 ; klebsiella 50/-5 | giant virus mai 15 50/-1
	def get_orthomcl_config(self, n, percent_match_cutoff, evalue_exponent):
...
oracleIndexTblSpc=NONE""" % (n, n, n, n, n, n, percent_match_cutoff, evalue_exponent)
	...
	def generate_config_file(self, tables_index, working_dir, percent_match_cutoff, evalue_exponent):
		config = self.get_orthomcl_config(tables_index, percent_match_cutoff, evalue_exponent)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-e", '--e_value_exp',type=str,help="e-value exponent cutoff for BLAST and orthomcl clustering (default=-5)", default="-5")
    parser.add_argument("-m", '--percent_match_cutoff',type=str,help="percent_match_cutoff for orthomcl clustering (default=50)", default="50")
    ...
    test = Orthomcl(args.input_fasta, args.index, working_dir, args.BLAST_file, args.e_value_exp, args.percent_match_cutoff, args.inflation_value)
