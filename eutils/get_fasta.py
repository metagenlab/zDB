...
def download_seq_entrez(accession, multifasta, seq_type, append):
    ...
    if multifasta=="":
        for one_accession in accession:
            output_name = re.sub("([a-zA-Z_0-9]+)\.([a-zA-Z]+)","\\1.fasta",one_accession)
     ...
     # if the user requests a single multifasta file
        ...
        if append != "":
            ...
            cmd = "cat %s %s_only.fasta > %s.fasta" % (append, multifasta, multifasta)
            print cmd
            shell_command(cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-p", '--append', type=str, default="", help="If required, name of the blast query file that should be appended to the output fasta file")
