...
def gc_coverage_plot(samtool_depth_file, contigs_file, blast_file=False, column1=1, column2=2, main=False, highlight=False):
    ...
    if (any("gc_coverage_subset" %%in%% ls())) {
     ...
     symbols(x=gc_coverage_table[,3], y= gc_coverage_table[,4], circles=gc_coverage_table[,2], inches=1/3, ann=F, bg=gc_coverage_table$color, fg=gc_coverage_table$color, add = TRUE, xlab="GC(%%)", ylab="Sequencing depth")
l <- gsub('(^[^_]+_[^_]+)_(.*)$', '\\\\1', gc_coverage_subset$Name)
    ...
    if (any("gc_coverage_subset" %%in%% ls())) {
     ...
     symbols(x=gc_coverage_table_2m[,3], y= gc_coverage_table_2m[,4], circles=gc_coverage_table_2m[,2], inches=1/3, ann=F, bg=gc_coverage_table_2m$color, fg=gc_coverage_table_2m$color, add = TRUE, xlab="GC(%%)", ylab="Sequencing depth")
