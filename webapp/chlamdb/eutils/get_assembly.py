    ...
    for seq_link in sublinks:
        ...
        while not one_handle:
            ...
            try:
                ...
                one_handle = Entrez.efetch(
                    db="nucleotide", id=seq_link, rettype="gbwithparts", retmode="text")
                # print "################################  features ##########################################"
        ...
        for record in seq_records:
            ...
            if record.seq.count("N") == len(record.seq):
                ...
                # try:
                one_handle = Entrez.efetch(
                    db="nucleotide", id=seq_link, rettype="fasta", retmode="text")
...
def get_wgs_links(one_species_link):
    ...
    handle = Entrez.elink(dbfrom="genome", db="nuccore",
                          id=one_species_link, term="wgs[prop]")
    record = Entrez.read(handle)
...
def multiple_wgs_links(ncbi_taxon):
    ...
    handle = Entrez.esearch(
        db="genome", term="txid%s[Organism:exp]" % ncbi_taxon, retmax=100000)
    record = Entrez.read(handle)
...
def get_complete_genomes_data(ncbi_taxon):
    ...
    for one_genome_id in genome_id_list:
        ...
        handle = Entrez.elink(dbfrom="genome", db="nuccore", id=one_genome_id,
                              term="gene+in+genomic[prop] OR gene+in+chromosome[prop]")
    ...
    for record_id in genome_record_id_list:
        ...
        while not handle_assembly:
            ...
            if i == 10:
                ...
                return
            try:
                handle_assembly = Entrez.elink(
                    dbfrom="nuccore", db="assembly", id=record_id)
            except (urllib2.URLError, urllib2.HTTPError) as e:
                print 'url error, trying again...'
                time.sleep(1)
                i += 1
        ...
        try:
            ...
            handle_sequences = Entrez.elink(
                dbfrom="assembly", db="nuccore", id=assembly_link, term="srcdb+ddbj/embl/genbank[prop]")
            ...
            handle = Entrez.efetch(
                db="nucleotide", id=record_id, rettype="gbwithparts", retmode="text")
            ...
            handle_bioproject = Entrez.elink(
                dbfrom="nuccore", db="bioproject", id=record_id)
            ...
            handle_refseq = Entrez.elink(
                dbfrom="bioproject", db="nuccore", id=bioproject_link)
            record_refseq = Entrez.read(handle_refseq)

            refseq_links = [link["Id"]
                            for link in record_refseq[0]["LinkSetDb"][0]["Link"]]
            ...
            for link in refseq_links:
                handle_genbank = Entrez.elink(
                    dbfrom="nuccore", db="nuccore", id=link, term="srcdb+ddbj/embl/genbank[prop]")
                record_genbank = Entrez.read(handle_genbank)
                sequences_links.append(
                    record_genbank[0]["LinkSetDb"][1]["Link"][0]["Id"])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", '--taxon_id', type=str,
                        help="get wgs link from taxonomic id")
    parser.add_argument("-w", '--wgs', type=str, help="download one wgs link")
    parser.add_argument("-c", '--complete_genomes', type=str,
                        help="taxonomic id (get complete genomes only")

    parser.add_argument("-a", '--all_genomes', type=str,
                        help="taxonomic id (get all genomes (wgs + complete)")
