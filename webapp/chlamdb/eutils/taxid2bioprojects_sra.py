...
def get_genomic_projects(taxon_id):
    ...
    for n, sra_list in enumerate(chunk_lists):
        ...
        project_id2sra_list = merge_dictionnaries(
            project_id2sra_list, temp_dico)
...
def sra2bioproject(sra_id_list):
    ...
    handle = Entrez.elink(dbfrom="sra", db="bioproject",
                          id=filter_id, retmax=100)
...
with open('sra_projects_bacteria.tab', 'w') as f:
    for n, project in enumerate(project2sra_list):
        ...
        if len(project2sra_list[project]) < 20:
            continue
        project_id, poject_name, project_title, description, registration_date = bioproject2data(
            gi(project, database="bioproject")[0])
