...
def cog_id2sequences(cog_id):
    ...
    sys.stdout.write('N sequences for cog %s: %s' %
                     (cog_id, len(prot_id_list)))
    handle = Entrez.efetch(db="nucleotide", id=','.join(
        prot_id_list), rettype="fasta", retmode="text")
...
if __name__ == '__main__':
    ...
    mafft_cmd = 'mafft --auto --maxiterate 1000 %s.fa > %s_mafft.fa' % (
        args.cog_id, args.cog_id)
