...
def get_taxi2assembly_accession(ncbi_taxon,
    ...
    for i, one_assembly in enumerate(assembly_id_list):
        ...
        contig_count = re.findall(
            '<Stat category="contig_count" sequence_tag="all">([^<]*)<', assembly_record['DocumentSummarySet']['DocumentSummary'][0]['Meta'])[0]
        status = re.findall(
            '<assembly-status>([^<]*)<', assembly_record['DocumentSummarySet']['DocumentSummary'][0]['Meta'])[0]
        repres = re.findall(
            'representative-status>([^<]*)<', assembly_record['DocumentSummarySet']['DocumentSummary'][0]['Meta'])[0]
        length = re.findall(
            'Stat category="total_length" sequence_tag="all">([^<]*)<', assembly_record['DocumentSummarySet']['DocumentSummary'][0]['Meta'])[0]
...
def get_complete_genomes_data(ncbi_taxon,
    ...
    handle = Entrez.esearch(db="assembly", term=s_term %
            ...
            try:
                ...
                if status == False:
                    if genbank:
                        ...
                        print('no RefSeq record for assembly %s' %
                              (one_assembly))
                        continue
            except IndexError:
                ...
                if genbank:
                    print(
                        'RefSeq link could not be identified, downloading GenBank record')
                    ftp_path = re.findall(
                        '<FtpPath type="GenBank">ftp[^<]*<', assembly_record['DocumentSummarySet']['DocumentSummary'][0]['Meta'])[0][50:-1]
                    get_ncbi_genome(ftp_path, out_dir)
                else:
                ...
                try:
                    print(
                        'RefSeq ftp link could not be found, downloading GenBank record')
...
def get_sample_data(accession):
    ...
    for i in record['DocumentSummarySet']['DocumentSummary'][0]:
        print("%s\t%s" %
              (i, record['DocumentSummarySet']['DocumentSummary'][0][i]))
...
def download_assembly(assembly_gi, out_dir, complete=False):
    ...
    print(assembly_record['DocumentSummarySet']['DocumentSummary'][0]['Meta'])
    print(assembly_record['DocumentSummarySet']
          ['DocumentSummary'][0]['PropertyList'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-a", '--accession', type=str,
                        help="Get assembly of a specific genbank/refseq accession")
    parser.add_argument("-c", '--complete', action="store_true",
                        help="complete genome only (default = False)")
    parser.add_argument("-s", '--sample', type=str,
                        help="Get sample data from accession id")
    parser.add_argument("-b", '--bioproject', type=str,
                        help="Get all assemblies from bioproject")
    ...
    parser.add_argument("-g", '--genbank', action="store_true",
                        help="Download only Genbank record if the assembly is not in RefSeq")
    parser.add_argument("-ta", '--taxid2assembly_accession', action="store_true",
                        help="get correspondance table from taxid2accession")
