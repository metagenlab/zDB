...
def import_phylo(phylo_list, biodb):
    ...
    sql = 'create table IF NOT EXISTS phylogenies (orthogroup varchar(100), phylogeny longtext);' % biodb
    ...
    for n, phylo in enumerate(phylo_list):
        ...
        #print t.write()
        sql = 'insert into phylogenies values ("%s", "%s");' % (biodb, orthogroup, t.write())
        #print sql
        server.adaptor.execute(sql,)
    server.commit()
