...
def collect_EC(db_name):
    ...
    all_EC_ids_sql = 'select distinct ec from enzyme_locus2ec_%s as t1 inner join enzyme.enzymes as t2 on t1.ec_id=t2.enzyme_id;' % (db_name)
...
def collect_EC_accession(db_name):
    ...
    for accession in all_ec_ids:
        ...
        sql = 'select t1.accession, count(*) as n from enzyme_locus2ec_%s as t1 inner join enzyme.enzymes as t2 ' \
